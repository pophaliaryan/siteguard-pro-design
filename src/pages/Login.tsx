  ...
  return <div className="min-h-screen bg-muted">
      ...
      <div className="relative h-64 md:h-80 overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
          <h1 className="text-4xl md:text-5xl font-heading font-bold text-primary-foreground mb-3 animate-fade-in">Vina Developers</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {roles.map((role, index) => <div key={role.id} style={{
          animationDelay: `${index * 100}ms`
        }}>
              <RoleCard title={role.title} description={role.description} icon={role.icon} onSelect={() => handleRoleSelect(role.id)} />
            </div>)}
        </div>
      </div>
